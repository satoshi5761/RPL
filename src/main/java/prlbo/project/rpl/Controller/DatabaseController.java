...
public class DatabaseController {
    ...
    public boolean TambahKategori(int idAccount, String namaKategori) {
        String query = "INSERT INTO kategori (id_account, namaKategori) VALUES (?, ?)";

        try (PreparedStatement stmt = con.prepareStatement(query)) {
            stmt.setInt(1, idAccount);
            stmt.setString(2, namaKategori);
            stmt.executeUpdate();
            System.out.println("Kategori berhasil ditambahkan.");
            return true;
        } catch (SQLException e) {
            e.printStackTrace();
            System.out.println("Kategori gagal ditambahkan.");
            return false;
        }
    }
